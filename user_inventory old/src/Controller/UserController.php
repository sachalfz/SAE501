<?php

namespace App\Controller;

use App\Entity\User;
use App\Entity\Inventory;
use App\Form\UserType;
use App\Repository\UserRepository;
use Doctrine\ORM\EntityManagerInterface;
use Symfony\Bundle\FrameworkBundle\Controller\AbstractController;
use Symfony\Component\PasswordHasher\Hasher\UserPasswordHasherInterface;
use Symfony\Component\HttpFoundation\Request;
use Symfony\Component\HttpFoundation\Response;
use Symfony\Component\Routing\Annotation\Route;

#[Route('/user')]
class UserController extends AbstractController
{
    #[Route('/', name: 'app_user_index', methods: ['GET'])]
    public function index(UserRepository $userRepository): Response
    {
        return $this->render('user/index.html.twig', [
            'users' => $userRepository->findAll(),
        ]);
    }

    #[Route('/new', name: 'app_user_new', methods: ['GET', 'POST'])]
    public function new(Request $request, EntityManagerInterface $entityManager, UserPasswordHasherInterface $passwordHasher): Response
    {
        $user = new User();
        $form = $this->createForm(UserType::class, $user);
        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {
            // Récupérer le mot de passe non-hashé
            $plainPassword = $user->getPassword();

            // Hasher le mot de passe
            $hashedPassword = $passwordHasher->hashPassword($user, $plainPassword);
            $user->setPassword($hashedPassword);


            // Création d'un nouvel inventaire pour l'utilisateur
            $inventory = new Inventory();

            // Associer l'inventaire à l'utilisateur
            $user->setIdInventory($inventory);

            // Initialiser les propriétés gamesplayed et gameswon à 0
            $inventory->setGamesplayed(0);
            $inventory->setGameswon(0);
            $inventory->setStreamz(0);
            $inventory->setItems([]);
            $inventory->setProfilepicture("https://icons.veryicon.com/png/o/internet--web/prejudice/user-128.png");
            $inventory->setUsername("Guest");

            $entityManager->persist($user);
            $entityManager->persist($inventory);
            $entityManager->flush();

            return $this->redirectToRoute('app_user_index', [], Response::HTTP_SEE_OTHER);
        }

        return $this->render('user/new.html.twig', [
            'user' => $user,
            'form' => $form,
        ]);
    }

    #[Route('/{id}', name: 'app_user_show', methods: ['GET'])]
    public function show(User $user): Response
    {
        return $this->render('user/show.html.twig', [
            'user' => $user,
        ]);
    }

    #[Route('/{id}/edit', name: 'app_user_edit', methods: ['GET', 'POST'])]
    public function edit(Request $request, User $user, EntityManagerInterface $entityManager): Response
    {
        $form = $this->createForm(UserType::class, $user);
        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {
            $entityManager->flush();

            return $this->redirectToRoute('app_user_index', [], Response::HTTP_SEE_OTHER);
        }

        return $this->render('user/edit.html.twig', [
            'user' => $user,
            'form' => $form,
        ]);
    }

    #[Route('/{id}', name: 'app_user_delete', methods: ['POST'])]
    public function delete(Request $request, User $user, EntityManagerInterface $entityManager): Response
    {
        if ($this->isCsrfTokenValid('delete'.$user->getId(), $request->request->get('_token'))) {
            $entityManager->remove($user);
            $entityManager->flush();
        }

        return $this->redirectToRoute('app_user_index', [], Response::HTTP_SEE_OTHER);
    }

    #[Route('/api/users', name: 'api_create_user', methods: ['POST'])]
    public function createUser(Request $request, EntityManagerInterface $entityManager, UserPasswordHasherInterface $passwordHasher): Response
    {
        // Récupérer les données JSON de la requête
        $data = json_decode($request->getContent(), true);

        // Créer un nouvel objet User
        $user = new User();
        // Peupler l'objet User avec les données reçues de la requête
        // Assurez-vous de valider, filtrer et traiter les données appropriées ici

        // Exemple :
        $user->setEmail($data['email']);
        $user->setPassword($data['password']);
        // ...

        // Hasher le mot de passe
        $hashedPassword = $passwordHasher->hashPassword($user, $user->getPassword());
        $user->setPassword($hashedPassword);

        // Créer un nouvel inventaire pour l'utilisateur
        $inventory = new Inventory();

        // Associer l'inventaire à l'utilisateur
        $user->setIdInventory($inventory);

        // Initialiser les propriétés de l'inventaire
        $inventory->setGamesplayed(0);
        $inventory->setGameswon(0);
        $inventory->setStreamz(0);
        $inventory->setItems([]);
        $inventory->setProfilepicture("https://icons.veryicon.com/png/o/internet--web/prejudice/user-128.png");
        $inventory->setUsername("Guest");

        // Persister et flusher l'utilisateur et l'inventaire
        $entityManager->persist($user);
        $entityManager->persist($inventory);
        $entityManager->flush();

        // Retourner une réponse indiquant la création réussie avec un statut HTTP approprié
        return new Response('User created', Response::HTTP_CREATED);
    }
}
